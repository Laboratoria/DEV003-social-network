  ...
  if (installationEntry.registrationStatus === 0) {
    if (!navigator.onLine) {
      const registrationPromiseWithError = Promise.reject(
        ERROR_FACTORY$1.create('app-offline')
      );
      return {
        installationEntry,
...
async function gtagOnConfig(
  gtagCore,
  initializationPromisesMap2,
  dynamicConfigPromisesList2,
    ...
    if (gtagParams && gtagParams['send_to']) {
      ...
      for (const sendToId of gaSendToList) {
        ...
        const initializationPromise =
          foundConfig && initializationPromisesMap2[foundConfig.appId];
...
function findGtagScriptOnPage(dataLayerName2) {
  ...
  for (const tag of Object.values(scriptTags)) {
    if (
      ...
      tag.src.includes(dataLayerName2)
    ) {
      return tag;
    }
  }
...
async function attemptFetchDynamicConfigWithRetry(
  ...
  try {
    ...
    if (measurementId) {
      logger.warn(
        `Timed out fetching this Firebase app's measurement ID from the server. Falling back to the measurement ID ${measurementId} provided in the "measurementId" field in the local Firebase config. [${
  ...
  try {
    ...
    if (!isRetriableError(error)) {
      ...
      if (measurementId) {
        logger.warn(
          `Failed to fetch this Firebase app's measurement ID from the server. Falling back to the measurement ID ${measurementId} provided in the "measurementId" field in the local Firebase config. [${
    ...
    const backoffMillis =
      Number(
        (_a =
          error === null || error === void 0 ? void 0 : error.customData) ===
          null || _a === void 0
          ? void 0
          : _a.httpStatus
      ) === 503
...
async function _initializeAnalytics(
  ...
  dynamicConfigPromise
    .then(config => {
      ...
      if (
        app2.options.measurementId &&
        config.measurementId !== app2.options.measurementId
      ) {
        logger.warn(
          `The measurement ID in the local Firebase config (${app2.options.measurementId}) does not match the measurement ID fetched from the server (${config.measurementId}). To ensure analytics events are always sent to the correct Analytics property, update the measurement ID field in the local config or remove it from the local config.`
        );
      }
    })
...
function factory(app2, installations, options) {
  ...
  if (!app2.options.apiKey) {
    if (app2.options.measurementId) {
      logger.warn(
        `The "apiKey" field is empty in the local Firebase config. This is needed to fetch the latest measurement ID for this Firebase app. Falling back to the measurement ID ${app2.options.measurementId} provided in the "measurementId" field in the local Firebase config.`
      );
    } else {
      throw ERROR_FACTORY.create('no-api-key');
    }
...
function registerAnalytics() {
  ...
  _registerComponent(
    new Component('analytics-internal', internalFactory2, 'PRIVATE')
  );
...
const firebaseConfig = {
  ...
  appId: '1:422725777:web:e6f78bea171ae687639730',
  measurementId: 'G-LLZECPWEL2',
};
